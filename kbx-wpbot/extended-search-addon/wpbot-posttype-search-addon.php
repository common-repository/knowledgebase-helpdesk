...
if ( ! defined( 'QCLD_wpCHATBOT_POSTTYPE_PLUGIN_URL' ) ) {
	define('QCLD_wpCHATBOT_POSTTYPE_PLUGIN_URL', plugin_dir_url(__FILE__));
}

if ( ! defined( 'QCLD_EXTENDED_IMG_URL' ) ) {
...
class wbpt_Admin_Area_Controller {
	...
	public function wppt_admin_menu(){
		if ( current_user_can( 'publish_posts' ) ){
			//var_dump(current_user_can( 'publish_posts' ));wp_die();
			add_menu_page( 'Bot - Extended Search', 'Bot - Extended Search', 'manage_options', 'wbpt-posttypesetting-page', array( $this, 'wbpt_setting_page' ), 'dashicons-search' );
	...
	public function wppt_register_plugin_settings(){
		...
		register_setting( 'qc-wppt-plugin-settings-group', 'wppt_disable_taxes' );
		...
		<form method="post" action="options.php">
			...
			<div class="wppt-settings-section" id="general_settings">
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row">Disable Selected Terms of Categories/Taxonomies</th>
						<td>
							<?php
							
								$term_array = !is_array(get_option('wppt_disable_taxes')) ? array() : get_option('wppt_disable_taxes');
								...
								if( !empty($taxonomies) ){
									foreach( $taxonomies as $taxonomy ){
										...
										if( !empty($terms) ){
							?>
								<h4>Select <?php echo $taxonomy->label; ?></h4>
								<?php foreach( $terms as $term ){ ?>
									<input type="checkbox" name="wppt_disable_taxes[]" value="<?php echo $term->term_id; ?>" <?php echo ((in_array($term->term_id, $term_array))?'checked="checked"':''); ?> /> <?php echo $term->name; ?>
							<?php } } } } ?>
...
register_activation_hook(kbx_adv_bot_plugin_main_file, 'qcld_wpbot_pt_search_options');
if(!function_exists('qcld_wpbot_pt_search_options')){
	function qcld_wpbot_pt_search_options(){
	
		if(!get_option('wppt_post_types')) {
			update_option('wppt_post_types', array('post','page', 'kbx_knowledgebase'));
		}
...
function generate_wppt_file(){
	...
	if(wp_mkdir_p( $uploads_dir )){
		...
		foreach ($sql as $key) {
			...
			$alt_title = maybe_unserialize(get_post_meta( $key->ID, 'kpm_more_queries', true));
