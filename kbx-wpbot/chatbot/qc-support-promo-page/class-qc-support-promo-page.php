...
if ( ! defined( 'qcpromo_chatbot_support_path' ) ) {
    define('qcpromo_chatbot_support_path', plugin_dir_path(__FILE__));
}

if ( ! defined( 'qcpromo_chatbot_support_url' ) )
    define('qcpromo_chatbot_support_url', plugin_dir_url( __FILE__ ) );

if ( ! defined( 'qcld_support_img_url' ) )
    define('qcld_support_img_url', qcpromo_chatbot_support_url . "/images" );


/*Callback function to add the menu */
function qcpromo_chatbot_show_promo_page_callback_func(){
    ...
    add_submenu_page(
        ...
        'qcpromo_support_page_callback_func'
    );
    
} //show_promo_page_callback_func

add_action( 'admin_menu', 'qcpromo_chatbot_show_promo_page_callback_func', 110 );
...
if ( ! function_exists( 'qcpromo_chatbot_include_promo_page_scripts' ) ) {	
	function qcpromo_chatbot_include_promo_page_scripts( ) {   


        if( isset($_GET["page"]) && !empty($_GET["page"]) && (   $_GET["page"] == "wpbot_support_page"  ) ){

            wp_enqueue_style( 'qcld-support-fontawesome-css', qcpromo_chatbot_support_url . "css/font-awesome.min.css");                              
            wp_enqueue_style( 'qcld-support-style-css', qcpromo_chatbot_support_url . "css/style.css");

            wp_enqueue_script( 'jquery' );
            wp_enqueue_script( 'jquery-ui-core');
            wp_enqueue_script( 'jquery-ui-tabs' );
            wp_enqueue_script( 'jquery-custom-form-processor', qcpromo_chatbot_support_url . 'js/support-form-script.js',  array('jquery', 'jquery-ui-core','jquery-ui-tabs') );

            wp_add_inline_script( 'jquery-custom-form-processor', 
                                    'var qcpromo_chatbot_ajaxurl    = "' . admin_url('admin-ajax.php') . '";
                                    var qcpromo_chatbot_ajax_nonce  = "'. wp_create_nonce( 'qc-clr' ).'";   
                                ', 'before');
            
        }
	   
	}
	add_action('admin_enqueue_scripts', 'qcpromo_chatbot_include_promo_page_scripts');
 ...
 *******************************/
if ( ! function_exists( 'qcpromo_support_page_callback_func' ) ) {
...
?>
        ...
        <div class="qc-clr-comments-support qcld-support-new-page">
            ...
            <?php include_once qcpromo_chatbot_support_path . '/qc-clr-recommendbot-support-plugin.php'; ?>
 ...
 *******************************/
add_action( 'wp_ajax_qcpromo_chatbot_process_qc_promo_form', 'qcpromo_chatbot_process_qc_promo_form' );

if( !function_exists('qcpromo_chatbot_process_qc_promo_form') ){

    function qcpromo_chatbot_process_qc_promo_form(){
