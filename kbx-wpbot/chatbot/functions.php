...
function wp_chatbot_check_opening_hours(){
    ...
    if(get_option('wpwbot_hours')) {
        $wpwbot_times = wp_kses_post(maybe_unserialize(get_option('wpwbot_hours')));
    ...
    if (get_option('qlcd_wp_chatbot_search_option') == 'standard') {
        ...
        if ($product_num > 0) {
            ...
            if ($total_product_num > $product_per_page && $product_per_page > 0 ) {
                $html .= '<p style="text-align: center"><button type="button" id="wp-chatbot-loadmore" data-offset="' . $product_per_page . '" data-search-type="product" data-search-term="' . $keyword . '" >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_load_more')))) . ' <span id="wp-chatbot-loadmore-loader"></span></button> </p>';
            }
        }
        $html .= '</div>';
        ...
        if ($product_num > 0) {
            ...
            $html .= '</ul>';
            if ($total_product_num > $product_per_page && $product_per_page > 0) {
                $html .= '<p style="text-align: center"><button type="button" id="wp-chatbot-loadmore" data-offset="' . $product_per_page . '" data-search-type="product" data-search-term="' . $more_product_ids . '" >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_load_more')))) . ' <span id="wp-chatbot-loadmore-loader"></span></button> </p>';
            }
        }
        $html .= '</div>';
    }
...
function qcld_wb_chatbot_sub_category(){
    $parent_id  = isset( $_POST['parent_id'] ) ? stripslashes($_POST['parent_id']) : '';
...
function qcld_wb_chatbot_category_products(){
    $category_id        = isset( $_POST['category'] ) ? stripslashes($_POST['category']) : '';
    ...
    if ($product_num > 0) {
        ...
        $html .= '</ul>';
        if ($total_product_num > $product_per_page && $product_per_page >0) {
            $html .= '<p style="text-align: center"><button type="button" id="wp-chatbot-loadmore" data-offset="' . $product_per_page . '" data-search-type="category" data-search-term="' . $category_id . '" >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_load_more')))) . ' <span id="wp-chatbot-loadmore-loader"></span></button> </p>';
        }
        $html .= '</div>';
    } else {
        $html = '';
...
function qcld_wb_chatbot_featured_products(){
    ...
    if ($product_num > 0) {
        ...
        if ($total_product_num > $product_per_page) {
            $html .= '<p style="text-align: center"><button type="button" id="wp-chatbot-loadmore" data-offset="' . $product_per_page . '" data-search-type="product" data-search-term="featured" >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_load_more')))) . ' <span id="wp-chatbot-loadmore-loader"></span></button> </p>';
        }
    }
    $html .= '</div>';
    $response = array('html' => $html, 'product_num' => $total_product_num, 'per_page' => $product_per_page);
...
function qcld_wb_chatbot_sale_products(){
    ...
    if ($product_num > 0) {
        ...
        $html .= '</ul>';
        if ($total_product_num > $product_per_page) {
            $html .= '<p style="text-align: center"><button type="button" id="wp-chatbot-loadmore" data-offset="' . $product_per_page . '" data-search-type="product" data-search-term="sale" >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_load_more')))) . ' <span id="wp-chatbot-loadmore-loader"></span></button> </p>';
        }
    }
    $html .= '</div>';
    $response = array('html' => $html, 'product_num' => $total_product_num, 'per_page' => $product_per_page);
...
function qcld_wb_chatbot_load_more(){
    $offset             = isset( $_POST['offset'] )         ? stripslashes($_POST['offset']) : '';
    $search_type        = isset( $_POST['search_type'] )    ? stripslashes($_POST['search_type']) : '';
    $search_term        = isset( $_POST['search_term'] )    ? stripslashes($_POST['search_term']) : '';
    $product_per_page   = sanitize_text_field(get_option('qlcd_wp_chatbot_ppp') != '' ? get_option('qlcd_wp_chatbot_ppp') : 10);
    $product_orderby    = sanitize_text_field(get_option('qlcd_wp_chatbot_product_orderby') != '' ? get_option('qlcd_wp_chatbot_product_orderby') : 'title');
    $product_order      = sanitize_text_field(get_option('qlcd_wp_chatbot_product_order') != '' ? get_option('qlcd_wp_chatbot_product_order') : 'ASC');
...
function qcld_wb_chatbot_product_details(){
    $product_id = isset( $_POST['wp_chatbot_pid'] )    ? stripslashes($_POST['wp_chatbot_pid']) : '';
    ...
    if ($product->is_in_stock()) {
        if ($product_type == "variable") {
            ...
            foreach ($attributes as $attribute) {
                ...
                if (!in_array($name, $varation_names)) {
                    ...
                    $variations .= '<option value="">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_choose_option')))) . '</option>';
        ...
        $product_quantity .= '<label for="">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_quantity')))) . '</label>
...
function qcld_wb_chatbot_variable_add_to_cart(){
    $product_id     = isset( $_POST['p_id'] )               ? stripslashes($_POST['p_id'])  : '';
    $quantity       = isset( $_POST['quantity'] )           ? stripslashes($_POST['quantity'])  : '';
    $variations_id  = isset( $_POST['variations_id'] )      ? stripslashes($_POST['variations_id'])  : '';
    $attrs          = isset( $_POST['attributes'] )         ? stripslashes($_POST['attributes'])  : '';
...
function qcld_wb_chatbot_add_to_cart(){
    $product_id         = isset( $_POST['product_id'] )    ? stripslashes($_POST['product_id']) : '';
    $product_quantity   = isset( $_POST['quantity'] )      ? stripslashes($_POST['quantity']) : '';
    global $wpcommerce;
...
function qcld_wb_chatbot_support_email(){
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        $response['message'] = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_invalid_email'))));
        $response['status'] = 'fail';
    } else {
        //build email body
        $bodyContent = "";
...
function qcld_wb_chatbot_login_user(){
    ...
    $info = array();
    $info['user_login']     = isset( $_POST['user_name'] )     ? trim(sanitize_text_field($_POST['user_name'])) : '';
    $info['user_password']  = isset( $_POST['user_pass'] )     ? trim(sanitize_text_field($_POST['user_pass'])) : '';
    $info['remember'] = true;
    $user_signon = wp_signon($info, false);
    $response = array();
    if (is_wp_error($user_signon)) {
        $response['status'] = "fail";
        $response['message'] = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_order_password_incorrect'))));
    } else {
...
function get_order_by_username($user_name){
    ...
    if ($response['order_num'] > 0) {
        $response['message'] .= wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_order_found'))));
        ...
        $order_html .= '<p>' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_order_not_found')))) . '</p>';
    }
...
function qcld_wb_chatbot_recently_viewed_products(){
    ...
    //show post per page.
    $product_per_page = sanitize_text_field(get_option('qlcd_wp_chatbot_ppp') != '' ? get_option('qlcd_wp_chatbot_ppp') : 10);
    $wp_chatbot_product_title = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_latest_product_welcome'))));
    ...
    if (!empty($viewed_products)) {
        $wp_chatbot_product_title = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_viewed_product_welcome'))));
        ...
        $wp_chatbot_product_title = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_featured_product_welcome'))));
...
function qcld_wb_chatbot_show_cart(){
    ...
    $cart_title = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_shopping_cart'))));
    $no_cart_item_msg = wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_no_cart_items'))));
    if (get_option('wp_chatbot_custom_agent_path') != "" && get_option('wp_chatbot_agent_image') == "custom-agent.png") {
        $wp_chatbot_custom_icon_path = get_option('wp_chatbot_custom_agent_path');
    } else if (get_option('wp_chatbot_custom_agent_path') != "" && get_option('wp_chatbot_agent_image') != "custom-agent.png") {
        $wp_chatbot_custom_icon_path = QCLD_wpCHATBOT_IMG_URL . get_option('wp_chatbot_agent_image');
    ...
    $html = '<div class="wp-chatbot-agent-profile">
            <div class="wp-chatbot-widget-avatar"><img src="' . esc_url($wp_chatbot_custom_icon_path) . '" alt=""></div>
            <div class="wp-chatbot-widget-agent">' . esc_html(get_option('qlcd_wp_chatbot_agent')) . '</div>
            <div class="wp-chatbot-bubble">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_welcome')))) . '</div>
            </div>';
    if ($itemCount >= 1) {
        $html .= '<div class ="wp-chatbot-cart-container">';
        $html .= '<div class="wp-chatbot-cart-header"><div class="qcld-wp-chatbot-cell">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_title')))) . '</div><div class="qcld-wp-chatbot-cell">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_quantity')))) . '</div><div class="qcld-wp-chatbot-cell">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_price')))) . '</div><div class="qcld-wp-chatbot-cell"></div></div>';
        ...
        $html .= '<div class="wp-chatbot-cart-footer"><div class="qcld-wp-chatbot-cart-page"><a class="wp-chatbot-cart-link" href="' . $cart_url . '" target="_blank"  >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_link')))) . '</a></div><div class="qcld-wp-chatbot-checkout"><a class="wp-chatbot-checkout-link" href="' . $checkout_url . '" target="_blank"  >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_checkout_link')))) . '</a></div></div>';
        $html .= ' </div>';
    } else {
        $html .= '<div class="wp-chatbot-cart-container">';
...
function qcld_wb_chatbot_cart_shortcode(){
    ...
    if ($itemCount >= 1) {
        ...
        $html .= '<div class="wp-chatbot-cart-footer"><div class="qcld-wp-chatbot-cart-page"><a href="' . site_url() . '/cart" target="_blank" >' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_cart_link')))) . '</a></div><div class="qcld-wp-chatbot-checkout"><a href="' . site_url() . '/checkout" target="_blank">' . wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_checkout_link')))) . '</a></div></div>';
        $html .= ' </div>';
    } else {
        $html .= '<div class="wp-chatbot-cart-container">';
        $html .= '<div><p style="text-align:center">' . $no_cart_item_msg . '</p></div>';
...
function qcld_wb_chatbot_update_cart_item_number(){
    ...
    $qnty           = isset( $_POST['qnty'] )               ? sanitize_text_field($_POST['qnty']) : '';
    global $wpcommerce;
...
function qcld_wb_chatbot_cart_page(){
    ...
    $html = "";
    if ($itemCount < 0) {
        $html .= wp_kses_post(wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_no_cart_items'))));
    } else {
