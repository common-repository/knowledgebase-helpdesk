...
function wpbo_search_site() {
	...
	if(get_option('enable_wp_chatbot_post_content') == 1){
		...
		$enable_post_types 	= array( 'post', 'page', 'kbx_knowledgebase');
		...
		$sql 		= $wpdb->prepare("SELECT * FROM ". $wpdb->prefix."posts where post_type in ('page', 'post', 'kbx_knowledgebase') and post_status='publish' and ((post_title LIKE %s)) order by ID DESC", '%' . $wpdb->esc_like($keyword) . '%');
	...
	if(!empty( $results )){
		...
		foreach ( (array) $q as $term ) {
			...
			$sql = $wpdb->prepare("SELECT * FROM ". $wpdb->prefix."posts where post_type in ('page', 'post','kbx_knowledgebase') and post_status='publish' and ((post_title LIKE %s)) order by ID DESC", '%'. $term .'%');
		...
		if(  !empty( $results) ){
			...
			foreach ($results as $value) {
				if(!empty($value[0]->guid)){
					//if($url_check[1] == $selected_lan){
						$total_post = $total_post + 1;
						$responses .='<div class="wpbot_card_wraper">';
						$responses .=	'<div class="wpbot_card_image '.( isset($featured_img_url) && $featured_img_url==''?'wpbot_card_image_saas':'').'"><a href="'.$value[0]->guid.'" '.( isset($new_window) && $new_window==1?'target="_blank"':'').'>';
						if(isset($featured_img_url) && $featured_img_url!=''){
			...
			$response['html'] .= $responses;
			$response['html'] .='</div>';
			if(isset($results->found_posts) && ($results->found_posts > $searchlimit ) ){
