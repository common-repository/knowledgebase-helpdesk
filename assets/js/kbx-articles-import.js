			...
			success: function( response ) {
				if ( response.success ) {
					...
					if ( 'done' === response.data.position ) {
						window.location = response.data.url + '&articles-imported=' + parseInt( $this.imported, 10 ) + '&articles-failed=' + parseInt( $this.failed, 10 ) + '&articles-updated=' + parseInt( $this.updated, 10 ) + '&articles-skipped=' + parseInt( $this.skipped, 10 );
					} else {
						$this.run_import();
					}
				}
