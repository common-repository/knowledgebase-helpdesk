...
if ( ! defined( 'qcld_kbxhd_support_path' ) ) {
    define('qcld_kbxhd_support_path', plugin_dir_path(__FILE__));
}

if ( ! defined( 'qcld_kbxhd_support_url' ) )
    define('qcld_kbxhd_support_url', plugin_dir_url( __FILE__ ) );

if ( ! defined( 'qcld_kbxhd_img_url' ) )
    define('qcld_kbxhd_img_url', qcld_kbxhd_support_url . "/images" );


/*Callback function to add the menu */
function qcld_kbxhd_show_promo_page_callback_func(){

    add_submenu_page(
        "edit.php?post_type=kbx_knowledgebase",
        esc_html__('More WordPress Goodies for You!'),
        esc_html__('Support'),
        'manage_options',
        "qcpro-promo-page-kbx-support",
        'qcld_kbxhd_promo_support_page_callback_func'
    );
    
} //show_promo_page_callback_func

add_action( 'admin_menu', 'qcld_kbxhd_show_promo_page_callback_func', 99 );
...
if ( ! function_exists( 'qcld_kbxhd_include_promo_page_scripts' ) ) {	
	function qcld_kbxhd_include_promo_page_scripts( ) {   


        if( isset($_GET["page"]) && !empty($_GET["page"]) && (   $_GET["page"] == "qcpro-promo-page-kbx-support"  ) ){

            wp_enqueue_style( 'qcld-support-fontawesome-css', qcld_kbxhd_support_url . "css/font-awesome.min.css");                              
            wp_enqueue_style( 'qcld-support-style-css', qcld_kbxhd_support_url . "css/style.css");

            wp_enqueue_script( 'jquery' );
            wp_enqueue_script( 'jquery-ui-core');
            wp_enqueue_script( 'jquery-ui-tabs' );
            wp_enqueue_script( 'jquery-kbxhd-form-processor', qcld_kbxhd_support_url . 'js/support-form-script.js',  array('jquery', 'jquery-ui-core','jquery-ui-tabs') );

            wp_add_inline_script( 'jquery-kbxhd-form-processor', 
                                    'var qcld_kbxhd_ajaxurl    = "' . admin_url('admin-ajax.php') . '";
                                    var qcld_kbxhd_ajax_nonce  = "'. wp_create_nonce( 'qc-clr' ).'";   
                                ', 'before');
            
        }
	   
	}
	add_action('admin_enqueue_scripts', 'qcld_kbxhd_include_promo_page_scripts');
...
include_once qcld_kbxhd_support_path . '/qc-clr-recommendbot-support-plugin.php';

if ( ! function_exists( 'qcld_kbxhd_promo_support_page_callback_func' ) ) {

	function qcld_kbxhd_promo_support_page_callback_func() {
		
?>


        <div class="qcld-kbxhd-support qcld-support-new-page">
            ...
            <div class="qcld-plugins-lists">
                <div class="qcld-plugins-loading">
                    <img src="<?php echo esc_url(qcld_kbxhd_img_url); ?>/loading.gif" alt="loading">
                </div>
            </div>
        </div>
 ...
 *******************************/
add_action( 'wp_ajax_qcld_kbxhd_process_qc_promo_form', 'qcld_kbxhd_process_qc_promo_form' );

if( !function_exists('qcld_kbxhd_process_qc_promo_form') ){

    function qcld_kbxhd_process_qc_promo_form(){
